    ...
    window.onload = function () {
        ...
        var displayIndex = function (imgSize, spacing, left, imgs, index, flat, width, titleBox) {
            ...
            if (imgs[index].dataset.info) {
                ...
                var link = imgs[index].dataset.url;
                titleBox.innerHTML = info;
                titleBox.setAttribute('href',link);
        ...
        var initCoverFlow = function (c) {
            ...
            document.querySelectorAll('.coverflow-title-box').forEach(box => box.remove());

            var imgSize   = window.innerWidth / 6 || 64,
                spacing   = window.innerWidth / 15 || 10,
            ...
            else c.style.height = (imgHeight + 180) + "px";
        ...
        window.addEventListener("resize",function(){
            for (var i = 0; i < coverflows.length; ++i) initCoverFlow(coverflows[i]);
        });
        }
})();